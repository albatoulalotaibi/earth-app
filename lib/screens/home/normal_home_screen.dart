// lib/screens/home/normal_home_screen.dart
import 'dart:async';
import 'dart:convert';
...
class _NormalHomeScreenState extends State<NormalHomeScreen> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    super.initState();
    _checkInitialToken(); // 👈 الفحص الفوري للتوكن أول ما تفتح الصفحة
    _searchController.addListener(_onSearch);
    _loadAssets(showLoading: true); 
    _startPolling();
  }
  ...
  // 👈 دالة جديدة للفحص السريع للتوكن وطرد المستخدم للإنترو فوراً
  ...
  Future<void> _loadAssets({required bool showLoading}) async {
    ...
    try {
      final uri = Uri.parse('https://ruba-gsh7.onrender.com/api/documents/?role=normal');
      SharedPreferences prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('auth_token');

      // 👈 إذا التوكن مختفي، اطرده بقوة
      ...
      // 👈 إذا السيرفر طرده، اطرده بقوة للإنترو
      ...
      if (response.statusCode == 200) {
        ...
        // 👈 حماية من الوميض: ما نسوي setState إلا إذا صدك اجت بيانات جديدة
        bool hasChanges = fetchedAssets.length != _allAssets.length;
        if (!hasChanges) {
          for (int i = 0; i < fetchedAssets.length; i++) {
            if (fetchedAssets[i].id != _allAssets[i].id) {
              hasChanges = true;
              break;
            }
          }
        ...
        if (mounted && (hasChanges || _isLoading)) {
          setState(() {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // 👈 التعديل السحري: إذا يكتب بالبحث تطلعله اللستة كبل
                    if (_searchController.text.isNotEmpty)
                      _buildSearchResults(isAr, isDark)
                    else ...[
                      // باقي الواجهة الأساسية (ما محذوف منها حرف)
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text(
                            isAr ? 'أصولك الرقمية' : 'YOUR DIGITAL ASSETS',
                          ...
                          GestureDetector(
                            onTap: () {
                              Navigator.pushNamed(
                                ...
                                arguments: null,
                              ).then((_) => _loadAssets(showLoading: true)); 
                            },
                            child: Text(
                      ...
                      Text(
                        isAr ? 'اعرف المزيد حول ما سيحدث\nلأصولك الرقمية' : 'LEARN MORE ABOUT WHAT WILL\nHAPPEN TO YOUR ASSETS',
                        style: AppTextStyles.heading3.copyWith(
                          color: isDark ? Colors.lightBlueAccent : AppColors.primaryBlue,
                          fontSize: 14,
                          fontWeight: FontWeight.w800,
                      ...
                      _buildInfoRow(
                        isAr ? 'الحذف' : 'Deletion',
                        isAr ? 'إذا اخترت الحذف، ستتم إزالة الأصل بشكل دائم من خوادمنا بعد انتهاء فترة عدم النشاط لضمان خصوصيتك التامة.' : 'If you choose deletion, the asset will be permanently removed from our servers after your inactive period ends to ensure your total privacy.',
                        isAr, isDark,
                      ),
                      const SizedBox(height: 12),
                      _buildInfoRow(
                        isAr ? 'نقل لمستفيد' : 'Transfer to beneficiary',
                        isAr ? 'يمكن تعيين الأصول لمستفيدين محددين، ليتم نقلها بعد وفاتك. سيتم نقل كافة الأصول الرقمية المحددة بسرية وأمان.' : 'Assets can be assigned to designated beneficiaries, allowing them to be transferred upon the asset holder\'s death. All selected digital assets will be securely transferred, ensuring confidentiality and security.',
                        isAr, isDark,
                      ),
                    ],
                  ],
  ...
  // 👈 دالة العرض الجديدة للبحث المباشر
  Widget _buildSearchResults(bool isAr, bool isDark) {
    ...
    return ListView.separated(
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(), // حتى يشتغل السكرول مال الصفحة الأساسية
      ...
      itemBuilder: (context, index) {
        ...
        return InkWell(
          onTap: () {
            ...
            ).then((_) => _loadAssets(showLoading: false));
          },
          borderRadius: BorderRadius.circular(12),
          child: Container(
