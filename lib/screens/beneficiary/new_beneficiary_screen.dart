// lib/screens/beneficiary/new_beneficiary_screen.dart
import 'dart:convert';
...
class NewBeneficiaryScreen extends StatefulWidget {
  const NewBeneficiaryScreen({Key? key}) : super(key: key);
...
class _NewBeneficiaryScreenState extends State<NewBeneficiaryScreen> {
  ...
  String? _validateEmail(String? v, bool isAr) {
    ...
    if (!RegExp(r'^[^@]+@[^@]+\.[^@]+').hasMatch(email)) {
      return isAr ? 'يرجى إدخال بريد صالح' : 'Please enter a valid email';
    }
    return null;
  }
  ...
  Future<void> _handleAdd(bool isAr) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isAr ? 'تمت إضافة المستفيد بنجاح!' : 'Beneficiary added successfully!'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isAr ? 'فشل الإضافة: ${response.statusCode}' : 'Failed to add: ${response.statusCode}'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(title: isAr ? 'مستفيد جديد' : 'NEW BENEFICIARY'),
      body: Padding(
